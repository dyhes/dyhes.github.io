...
2. **混合中英文分词** 若文本包含中英文混合内容，可配置多语言分词策略：
...
2. **索引生命周期管理（ILM）** 结合冷热分层策略，动态控制冗余字段的生命周期：
   - **热数据阶段**：全字段冗余（7 天）
   - **温数据阶段**：仅保留关键字段（30 天）
...
2. **禁用 `doc_values` 的后果** 若设置为 `false`，排序操作将无法执行，并返回以下错误：
...
2. **嵌套对象查询** 支持嵌套字段（Nested Fields）的查询：
...
1. **响应结构** 返回结果按操作提交顺序排列，每个操作的状态独立，例如：
