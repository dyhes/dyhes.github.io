...
- **Fury 框架** Fury 对 `record` 的序列化进行了深度优化：
...
2. **刷新策略** 默认更新操作不立即刷新索引（近实时搜索），若需强制刷新可设置：
...
3. **空值处理** 可变参数允许传入 `null`，需在方法内部做空指针检查：
...
1. **连接池配置** 在 `application.yml` 中定义连接参数，优化性能：
...
- **自定义查询** 通过 `@Query` 注解可直接编写 Redis 命令，例如：
...
- **显式设置过期时间** 在实体类中直接指定 `expiration`（单位：秒）：

  ```java
  @RedisHash(value = "myEntity", expiration = 3600) // 1小时后过期
  ```

- **动态调整过期时间** 通过 `RedisTemplate` 手动设置或更新过期时间：
