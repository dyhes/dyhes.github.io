...
1. **比较对象不同**
   ...
   - **`==`**：比较两个对象的**值是否相等**（通过调用对象的 `__eq__()` 方法实现）。
...
3. **自定义对象（可自定义深浅拷贝）**
   ...
   - **默认行为（浅拷贝）**：复制成员变量值（含指针地址），不复制指针指向的对象：
...
2. **替代方案**：
   ...
   更推荐用`@property`\+ 单下划线（如`_data`）实现安全封装，既能隐藏细节又可添加逻辑验证：
...
2. **非私有性**
   ...
   - `__var__` 不是私有成员！它与单下划线（`_var`）或双下划线前缀（`__var`）不同：
...
1. **参数顺序强制** 若 `*args` 后需指定关键字参数，必须显式命名：
...
2. **参数顺序规则**
   - **在函数定义中，**`**kwargs` 必须位于参数列表的**最后**，顺序为：**普通参数 → `*args` → `**kwargs`**
...
2. **默认值与安全访问**
   - 使用 `dict.get()` 避免 `KeyError` 或设置默认值：
...
- **多进程替代：使用**`multiprocessing`模块绕过GIL，每个进程独立GIL。
...
2. **线程本地数据（Thread-Local）** 为每个线程维护独立数据副本：
