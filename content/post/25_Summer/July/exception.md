...
2. **避免空 catch 块** 即使捕获异常，也需记录日志或提供降级逻辑，而非忽略：
   ```
   try {
       saveToDatabase(data);
...
1. **`try-catch` 捕获处理** 在方法内部直接处理异常逻辑：
   ```
   public void readFile() {
       try {
...
1. **资源管理**
   - **必须关闭流：使用**`try-with-resources`（Java 7+）自动关闭，防止资源泄漏
     2,4：
     ```
...
- **`InputStreamReader`**：解码字节流 → 字符流（需指定编码）
  ```
  // 显式指定UTF-8编码读取文本
  try (Reader reader = new InputStreamReader(
          new FileInputStream("data.txt"), StandardCharsets.UTF_8)) { ... }
  ```
- **`OutputStreamWriter`**：编码字符流 → 字节流
  ```
  // 显式指定GBK编码写入文本
...
- **UTF-16代理对（Surrogate Pairs）**： 辅助平面字符（如`U+1F600`）通过两个16位码元表示：
  - 高位代理：`0xD800–0xDBFF`
  - **低位代理：`0xDC00–0xDFFF`**
...
1. **多语言支持**
   ...
   - **编程语言：Python 3默认使用Unicode字符串（**`str`类型），可直接处理多语言文本：
     ```
...
- Selector 轮询：通过`select()`阻塞直到至少一个 Channel 就绪，再处理事件
  2,6：
  ```
...
- **`mmap`（内存映射文件）**：将文件映射到内存，直接操作内存避免用户态-内核态拷贝
  3：
  ```
...
- **`struct kevent`**：事件描述单元，定义监控目标与条件：
  ```
  struct kevent {
