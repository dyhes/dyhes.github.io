...
2. **核心组件与功能** Spring Cloud 的组件可分为两类：
...
- **启用负载均衡**：
  通过`@LoadBalanced`注解激活`RestTemplate`的负载均衡能力
  1,8：
...
2. **服务发现集成** 结合 Nacos/Eureka 实现动态路由：
...
- Feign
仅支持`@RequestLine`、`@Param`等原生注解，需额外适配 Spring MVC 接口，开发体验割裂
  2,7
  。
...
- 同时配置`blockHandler`和`fallback`时：
...
- **步骤**：
  ...
  2. **配置注册中心（如Nacos）和存储模式（推荐**`db`模式高可用）：
...
- **`@GlobalTransactional`仅管理全局事务边界**，每个参与者的数据库操作仍需标注 `@Transactional`，确保本地操作的原子性。
