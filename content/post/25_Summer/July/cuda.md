...
- **实现方法**：
  - **协作组（Cooperative Groups）**：
    支持动态定义线程组（如`tiled_partition`），并通过`sync()`同步。
  ...
  - **线程束洗牌函数（Warp Shuffle）**：
    通过寄存器直接交换数据，隐式实现同步（如`__shfl_down_sync()`）。
...
- **实现方式**：
  通过掩码指定需同步的线程（例如`0x0000FFFF`同步低16线程，
...
- **动态子组创建**： 可运行时按需划分任意大小的子组（如16或8线程）：
...
- **资源竞争导致阻塞**：若1024线程块消耗过多资源（如寄存器、共享内存），SM可能无法驻留足够线程块。例如：
...
- **扩容优化**：扩容时（`resize()`），元素新位置仅需判断`hash & old_capacity`的结果：
...
2. **向量化内存访问**
   - **用**`float4`/`int4`类型单次读写4个元素，提升带宽利用率。
