...
#### **1. 内核逻辑地址（Kernel Logical Address）**
...
#### **2. 内核虚拟地址（Kernel Virtual Address）**
...
- 
  ...
  - RAID控制器将LBA映射到多块磁盘的物理扇区。例如：
    ...
    - 
      ...
      ：磁盘控制器将LBA转换为柱面（C）、磁头（H）、扇区（S）三维地址：
...
1. **编程模型**
   ...
   - 
     ...
     ：逻辑简单，适合阻塞或非阻塞 I/O。开发者无需担心事件遗漏，未处理完的数据会持续触发通知。
