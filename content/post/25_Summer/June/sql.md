...
1. **空值处理**：
   - **汇总行的**`NULL`是占位符，需用`GROUPING()`函数区分实际空值与汇总标识：
...
- **动态维度：使用**`GROUPING SETS`替代CUBE，仅计算所需组合：
...
- **`GROUPING()`**：标识某列是否参与当前分组（0=参与，1=未参与）。
  示例：区分真实`NULL`与汇总占位符：
...
- 解决：使用`GROUPING()`或`COALESCE()`显式标识
  7：
...
- **`EXISTS`优势场景：
  当
 子查询表大、主表小**时， `EXISTS` 通过短路机制减少扫描量（例：主表 1 万行，子表 100 万行）。
...
- **`IN`优势场景：
  当
 子查询表小、主表大**时， `IN` 仅需一次子查询扫描（例：子查询返回 100 个固定值）。
...
- **`IN`的限制**：
  多数数据库（如 Oracle、SQL Server）不支持多列直接`IN`（需用元组语法）：
...
1. **显式指定参数类型与长度**
   - **可变长度类型**
     （如`varchar`）：必须指定长度（如`SqlDbType.VarChar, 50`），避免因值长度变化导致执行计划无法复用
     3
     。
...
2. **NULL值处理**
   - **使用**`LEFT JOIN`时，源表字段可能为`NULL`，需用`IFNULL()`或`COALESCE()`设置默认值：
