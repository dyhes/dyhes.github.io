...
- **`:authority`** 替代 `Host` 字段，但为兼容HTTP/1：
...
- 公钥被替换：攻击者伪造发送方身份，提供自己的公钥 → 接收方用错误公钥解密签名，得到的 `H` 无效。
...
- **加密**（发送方）： 明文 `M` 编码为曲线上的点 → 选择随机数 `r` → 计算：
...
- **生成预主密钥（Pre-Master Secret）**： 根据密钥交换算法生成：
