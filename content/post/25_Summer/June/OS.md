...
2. **软件屏蔽** 操作系统提供API控制中断，例如Linux内核的：
...
2. **若无 Partial Slab，使用 Empty Slab**：
...
- **逻辑块映射：文件系统将文件划分为**
  逻辑块
  （如4KB），并通过元数据（如ext4的`inode`索引或NTFS的
  主文件表MFT
  ）记录逻辑块的位置。
...
1. **编程模型**
   ...
   - **ET**：
     必须使用非阻塞 I/O
     ，且需循环读写直至返回`EAGAIN`或`EWOULDBLOCK`，否则会丢失后续数据。
