...
2. **逃逸分析（Escape Analysis）** ➠ 判断对象作用域，触发以下优化：
...
1. **委派阶段（自下而上）** 当子类加载器（如应用类加载器）收到加载请求时：
   - 检查该类是否已被加载（缓存）。
...
- **Spring容器：通过**`ClassMetadata`接口解析类信息，支持依赖注入。
...
2. **方法调用的高效定位**
   ...
   - 运行时：调用指令（如 invokevirtual）直接携带索引值，无需查找方法名。
