...
- 屏蔽表结构变更：当基表结构变化（如分表、增删列），可通过修改视图保持应用层不变。
  示例：基表`Student`拆分为`SX`（学号、姓名）和`SY`（性别、年龄），创建视图还原原结构：
  ```
...
- **FLOAT/DOUBLE（浮点数）**
  ...
  - 精度问题：二进制无法精确表示某些十进制小数（如`0.1`），导致
    舍入误差
    。例如：
    ```
...
1. **整数与小数分离**
   - 例如`DECIMAL(16,4)`存储`1234567890.1234`时，实际存储三个整数：
...
- **范围查询（如**`WHERE time_col > '2024-01-01'`）时：
...
3. **替代方案**
   - **需极致性能或解决2038问题：用**`BIGINT`存储Unix时间戳（毫秒级），由应用层处理格式和时区。
     ```
     CREATE TABLE events (
