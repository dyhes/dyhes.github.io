...
- **表锁替代：通过**`LOCK TABLES`锁定表防止并发干扰，但无法回滚。
...
- **方法：将UUID转换为16字节的** `BINARY(16)`，而非 `CHAR(36)`：
...
- 原理：调整UUID的字节顺序，将时间戳前置（如MySQL 8.0的 `UUID_TO_BIN(uuid, 1)`）：
...
- **避免全表扫描**： 对高频查询字段建立二级索引，并利用覆盖索引避免回表：
...
1. **回表查询（关键步骤）** 通过二级索引查找数据需**两次B+树搜索**：
