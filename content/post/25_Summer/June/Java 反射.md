...
1. **框架开发** Spring、Hibernate等框架利用反射实现：
...
2. **类型捕获（Type Capture）** 通过泛型方法捕获通配符的具体类型，避免强制转换：
...
1. **`instanceof` 关键字** 检查对象是否属于特定类或其子类：
...
2. **`getClass()` 方法** 返回对象的运行时类对应的 `Class` 对象：
...
3. **结合多态扩展功能** 当子类有特殊方法时，通过 RTTI 识别并调用：
