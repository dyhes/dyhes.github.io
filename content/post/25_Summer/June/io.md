...
- **数据结构：使用动态数组**`struct pollfd[]`，每个元素包含：
  - `fd`：监控的文件描述符
...
> - **设计意图**：epoll通过保留红黑树节点避免重复注册的开销，仅当明确要求（如`EPOLLONESHOT`）或连接关闭时才删除节点，以最大化性能[1,5](@ref)。
> - ​**实践建议**​：
