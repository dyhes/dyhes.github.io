...
- 端口未开放：客户端请求未监听的端口时，服务器直接回复RST拒绝握手。
  ```
  sequenceDiagram
      客户端->>服务器： SYN（请求连接）
      服务器->>客户端： RST（端口未开放）
