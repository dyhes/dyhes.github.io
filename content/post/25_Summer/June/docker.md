...
2. **产物复制机制**
   - 通过 `COPY --from=<stage-name>` 将前一阶段的文件复制到当前阶段，其他中间文件（如编译器、源码、临时文件）被丢弃。
...
```
   ...
   - **内存**：通过 `memory.limit_in_bytes` 设置内存上限，超限触发 OOM（Out-of-Memory）终止进程
   ...
   - **I/O**：通过 `blkio.throttle.read_bps_device` 限制磁盘读写速率
