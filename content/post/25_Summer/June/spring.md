...
- 示例：
 ...
 匹配 `com.example.service` 包下所有类的 save 开头的 public 方法，参数任意
...
- 示例： `@annotation(com.example.anno.Log)` 匹配所有标注了 @Log 注解的方法。
...
- 自动配置类：
 以 `DataSourceAutoConfiguration` 为例，其逻辑如下：
...
- Starter 的作用：
 每个 Starter（如 `spring-boot-starter-data-jpa` ）包含：
...
2. 在 `src/main/resources/META-INF/spring.factories` 中声明配置类：
...
- 作用原理 `@EnableTransactionManagement` 会注册关键组件：
...
2. 事务传播行为的触发条件
 当调用 `@Transactional` 方法时，Spring会检查
   当前线程的事务状态
   ，而非仅限于嵌套调用。例如：
...
- 默认加入外层事务
 若被调方法无 `@Transactional` 注解，则默认使用传播行为 `Propagation.REQUIRED` ，即：
...
- 回滚规则：默认仅回滚 `RuntimeException` 和 `Error` 。若需回滚受检异常（如 `IOException` ），需显式配置：
...
- Spring默认对未捕获的 `RuntimeException` 或 `Error` 自动回滚
  1,7,8
  。
...
- 若重抛的是检查型异常（如 `IOException` ），
  默认不会触发回滚
 ，除非显式配置 `rollbackFor` 3,8
  。
...
- 需手动添加事务：
 若通过 `Impl` 后缀类实现自定义逻辑（如 `UserRepositoryImpl` ），其中的方法不会自动继承事务，需显式标注 `@Transactional`
...
- 自定义注解：通过 AOP 解析注解中的过期参数
 ...
 实现原理：重写 `RedisCacheManager` ，在 `createRedisCache()` 方法中解析 `value` 中的
