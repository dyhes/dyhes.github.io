...
- **Maven/Gradle 的依赖传递机制**
  ...
  即使 Milvus SDK 声明了 FastJSON 依赖，若其作用域为`provided`或`compileOnly`，构建工具
...
2. **数据类型与索引类型的映射** Milvus 根据字段数据类型自动选择索引算法，对应关系如下：
...
4. **稀疏向量索引（SPARSE_INVERTED_INDEX）**
   - **结构**：维度级倒排列表（Dimension-wise Inverted Lists）。
...
1. **倒排索引（INVERTED）**
   - **结构**：词项字典（Term Dictionary）+ 倒排列表（Posting Lists）。
...
1. **数据类型匹配**
   - **查询条件必须与**`json_cast_type`一致，否则索引失效：
